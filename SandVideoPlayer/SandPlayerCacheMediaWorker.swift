...
import Foundation
import UIKit
open class SandPlayerCacheMediaWorker: NSObject {
    open fileprivate(set) var cacheConfiguration: SandPlayerCacheMediaConfiguration?
    ...
    public init(url: URL) {
        let path = SandPlayerCacheManager.cacheFilePath(for: url)
        filePath = path
        ...
        if err == nil {
            ...
            if err == nil {
                ...
                do {
                    ...
                    cacheConfiguration = SandPlayerCacheMediaConfiguration.configuration(filePath: path)
    ...
    open func cachedDataActions(forRange range:NSRange) -> Array<SandPlayerCacheAction> {
        var actions = [SandPlayerCacheAction]()
            ...
            for (_, value) in cachedSegments.enumerated() {
                ...
                if intersctionRange.length > 0 {
                    ...
                    for i in 0...package {
                        ...
                        let action = SandPlayerCacheAction(type: .local, range: ra)
        ...
        if actions.count == 0 {
            let action = SandPlayerCacheAction(type: .remote, range: range)
            actions.append(action)
        } else {
            var localRemoteActions = [SandPlayerCacheAction]()
            for (index, value) in actions.enumerated() {
                ...
                if index == 0 {
                    if range.location < actionRange.location {
                        ...
                        let action = SandPlayerCacheAction(type: .remote, range: ra)
    ...
    open func set(cacheMedia: SandPlayerCacheMedia) -> Bool {
